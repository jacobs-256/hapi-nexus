...
function getEmbeddedAssetsManifestPath(workspaceRoot: string): string {
    return join(workspaceRoot, 'hub', 'src', 'web', 'embeddedAssets.generated.ts');
}

function writeStubEmbeddedAssets(workspaceRoot: string): void {
    const outputPath = getEmbeddedAssetsManifestPath(workspaceRoot);
    const contents = [
...
function ensureEmbeddedAssetsManifest(workspaceRoot: string, includeWebAssets: boolean): void {
    const manifestPath = getEmbeddedAssetsManifestPath(workspaceRoot);
    if (includeWebAssets) {
...
async function buildServerAndClient(projectRoot: string, workspaceRoot: string, targets: string[], outdir: string): Promise<void> {
    const manifestPath = getEmbeddedAssetsManifestPath(workspaceRoot);
    ensureEmbeddedAssetsManifest(workspaceRoot, true);
    const embeddedAssetsManifest = readFileSync(manifestPath, 'utf-8');
...
async function main(): Promise<void> {
    ...
    if (args.includes('--target') && !target) {
        ...
        console.error('   or: bun run scripts/build-executable.ts [--server-and-client] [--target <bun-platform[-arch[-variant]]>] [--all] [--outdir dist-exe]');
        process.exit(1);
    }
    ...
    if (buildServerClient) {
        await buildServerAndClient(projectRoot, workspaceRoot, targets, outdir);
        return;
    }
